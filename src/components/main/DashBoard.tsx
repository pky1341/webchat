...
export const DashBoard = async () => {
  const { data, error } = useSWR("/api/users/get-users", fetcher);
    ...
    <div className="flex h-screen bg-gray-900 text-white">
      <aside className="w-1/4 bg-gray-800 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto">
          {data.length > 0 ? (
            data.map((user:User)=>(
            <div key={user.id} className="flex items-center p-4 border-b border-gray-700 hover:bg-gray-700">
              <Avatar>
                <AvatarImage src={user.image || "/placeholder-user.jpg"} alt="Avatar" />
                <AvatarFallback>U</AvatarFallback>
              </Avatar>
              <div className="flex-1 ml-4">
                <div className="flex justify-between">
                  <h2 className="font-bold">{user.username||"Unknown User"}</h2>
